...
mkdir -p ./.tuna_temp
wget -O ./.tuna_temp/magurohttp.zip https://download.magurohttp.eu/magurohttp_r2_linux64.zip

echo "Installing"
unzip -o ./.tuna_temp/magurohttp.zip -d ./.tuna_temp
cp ./.tuna_temp/magurohttp /usr/bin/magurohttp
cp ./.tuna_temp/magurohttp.service /etc/systemd/system/magurohttp.service
setcap cap_net_bind_service=+ep /usr/bin/magurohttp
mkdir -p /usr/lib/magurohttp/www

if [ ! -e /usr/lib/magurohttp/main.json ]; then
  cp ./.tuna_temp/main.json /usr/lib/magurohttp/main.json
else
  cp -f ./.tuna_temp/main.json /usr/lib/magurohttp/main.json.new
fi
